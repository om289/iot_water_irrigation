...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (forceToggle) {
    ...
    forceToggle.addEventListener('change', async (e) => {

      const forceOn = e.target.checked;

      if (forceOn) {

        if (typeof pumpActive !== 'undefined' && !pumpActive) triggerPump('Force On');

      } else {

        if (typeof pumpActive !== 'undefined' && pumpActive) {

          pumpActive = false;

          if (typeof updatePumpUI === 'function') updatePumpUI(false);
      ...
      if (hardwareIp && isConnectedToHardware) {
        ...
        try {
          ...
          await fetch(`http://${hardwareIp}/api/force?state=${forceOn}`, { method: 'POST' });

          console.log('Hardware force-pump set to:', forceOn);

        } catch (err) {

          console.error('Failed to update hardware force-pump:', err);
...
window.addSystemLog = function(module, message, type = 'info') {
  ...
  if (consoleEl) {
    ...
    let color = '#a5d6a7';
    if (type === 'warn') color = 'var(--amber-primary)';
    else if (type === 'error') color = 'var(--red-primary)';
    else if (type === 'success') color = 'var(--green-primary)';
    else if (type === 'system') color = 'var(--blue-primary)';
    
    log.style.color = color;
    log.textContent = `[${time}] [${module}] ${message}`;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', () => {
      const consoleEl = document.getElementById('logs-console');
      if (consoleEl) {
        consoleEl.innerHTML = '<div class="log-entry" style="color: var(--text-muted);">[System] Console cleared.</div>';
      }
    });
  }
});
