  ...
  if (hardwareIp && isConnectedToHardware) {
    try {
      ...
      if (response.ok) {
        ...
        if (window.addSystemLog) window.addSystemLog('Hardware', 'Successfully read data from NodeMCU at ' + hardwareIp, 'success');
      }
    } catch (e) {
      ...
      if (window.addSystemLog) window.addSystemLog('Hardware', 'Failed to connect to NodeMCU', 'error');
    }
  } else {
    // Simulated behavior
...
function updateSoilUI() {
  ...
  // Rate limit log output to avoid console spam (log every 3rd update or when state changes)
  if (!window.lastLogMoisture || Math.abs(window.lastLogMoisture - pct) >= 2) {
    window.lastLogMoisture = pct;
    if (window.addSystemLog) {
      const statusText = pct < 20 ? 'CRITICALLY DRY' : pct < 35 ? 'Thirsty' : pct < 70 ? 'Optimal' : 'Saturated';
      const logType = pct < 20 ? 'error' : pct < 35 ? 'warn' : pct < 70 ? 'info' : 'success';
      window.addSystemLog('Sensor', `Soil moisture level is ${pct}% (${statusText})`, logType);
    }
  }
}
