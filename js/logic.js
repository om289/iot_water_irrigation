...
function triggerPump(source) {
  ...
  if (window.addSystemLog) {
    window.addSystemLog('Pump', `Watering sequence triggered via ${source} for ${entry.duration}`, 'success');
  }
  ...
  setTimeout(() => {
    ...
    updatePumpUI(false);
    if (window.addSystemLog) {
      window.addSystemLog('Pump', 'Watering completed. Pump turned OFF.', 'system');
    }
  }, dur);
}
